    ...
    <div class="row">
        @foreach ($course_data as $course)
            <div class="col-md-4 thumbnail-container">
                    <img style="height: 220px" src="{{ $course -> course_img }}" alt="Avatar" class="thumbnail-image ">
                    <div class="thumbnail-middle">
                        <h1 class="thumbnail-title"> {{ $course -> course_name }} </h1>
                        <h1 class="thumbnail-subtitle">{{ $course -> course_des }} </h1>
                        <h1 class="thumbnail-subtitle">{{ $course -> course_class }} </h1>
                        <a href="{{ $course -> course_link }}" target="_blank" class="normal-btn btn">শুরু করুন</a>
                    </div>
            </div>
        @endforeach
    </div>
